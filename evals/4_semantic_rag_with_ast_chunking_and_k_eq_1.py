...
- AST-based code chunking (one chunk per class or module-level function)
...
Workflow: Load code → Chunk (AST) → Build vector store → Create agent → Evaluate on test cases
"""
...
from helpers.chunking import chunk_code_ast

# Import test cases for evaluation
from golden_dataset import TEST_CASES

# ============ CONFIGURATION CONSTANTS ============
# AST-based chunking: one chunk per class or module-level function
# Provides semantic coherence over fixed-size chunking

# Retrieval: k=1 means retrieve top 1 most relevant code chunks for each query
RETRIEVAL_K = 1
...
AGENT_RESPONSES_CACHE_FILE = CACHE_DIR / "agent_responses_cache_ast_chunking_k_1.json"
RAGAS_RESULTS_CACHE_FILE = CACHE_DIR / "ragas_results_cache_ast_chunking_k_1.pkl"
...
def build_agent(vector_store: Chroma):
    ...
    """
    # Create retriever tool that searches for k=1 most relevant code chunks
...
def print_scorecard(results):
    ...
    # Print report header
    print("\nRAGAS SCORECARD — Semantic RAG (AST Chunking, k=1) on sample_project")
...
def main():
    ...
    2. Chunk code using AST-based parsing (one chunk per class/function)
    ...
    parser = argparse.ArgumentParser(
        description="Semantic RAG using AST-based chunking and OpenAI LLM"
    )
    parser.add_argument(
        "--repo",
    ...
    # Chunk code using AST parsing to extract classes and functions as semantic units
